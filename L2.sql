...
,i.invoice_status
,ROW_NUMBER() OVER (PARTITION BY i.contract_id ORDER BY date_issue) AS invoice_order
,i.date_issue
,i.due_date
,i.paid_date
,i.start_date
...
where product_category in ( 'product','rent')
 and product_status_name is not null and product_status_name not in ('canceled','disconnected');
